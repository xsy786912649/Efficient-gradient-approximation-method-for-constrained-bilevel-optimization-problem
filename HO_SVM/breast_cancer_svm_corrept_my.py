    ...
    for j in range(30):
        ...
        for i in range(y_train.shape[0]):
            if i in active_constraint_list:
                ...
                #input()
        print(no_strictly_active)

        if len(no_strictly_active)>0 and len(no_strictly_active)<7:
            d1_list=[]
            #for num in range(int(math.pow(2,len(no_strictly_active)))):
            strict_active_list=[x for x in active_constraint_list if x not in no_strictly_active]
            power_set_of_no_strictly_active=subsets(no_strictly_active)
            for subset in power_set_of_no_strictly_active:
                v1=np.ones((feature,))
                v2=np.zeros((1,))
                v3=c_array_tensor.detach().numpy()
                M1= np.diag(np.hstack((v1,v2,v3)))
                ...
                M2_list=[]
                for i in range(y_train.shape[0]):
                    if i in strict_active_list or i in subset:
                        M2_list.append( np.array([ np.hstack((z_train[i]* (-y_train[i]),np.array([-y_train[i]]),-np.eye(y_train.shape[0])[i])) ]) )
                M2= np.vstack(M2_list)
                M3= np.transpose(M2)
                M4 = np.zeros((len(M2_list),len(M2_list)))
                M = np.hstack((np.vstack((M1,M2)), np.vstack((M3,M4))))
                #print(M.shape)
                #print(np.linalg.matrix_rank(M))
                ...
                n1=np.zeros((feature+1, y_train.shape[0]))
                n2=np.diag(np.array(xi.value)*c_array_tensor.detach().numpy())
                n3=np.zeros((len(M2_list),y_train.shape[0]))
                N=np.vstack((n1,n2,n3))
                #print(N.shape)
        ...
        c_array_1=c_array-alpha*grad_update
        c_array_tensor_1=torch.exp(c_array_1)
        C.value=c_array_tensor_1.detach().numpy()
        prob.solve(solver='ECOS', abstol=1e-25,reltol=1e-20,max_iters=1000000000, warm_start=True)
        w_tensor_1=torch.Tensor(np.array([w.value]))
        b_tensor_1=torch.Tensor(np.array([b.value]))
        x = torch.reshape(torch.Tensor(y_val), (torch.Tensor(y_val).shape[0],1)) * F.linear(torch.Tensor(z_val), w_tensor_1, b_tensor_1) / torch.linalg.norm(w_tensor_1)
        loss_upper_1= torch.sum(2*torch.sigmoid(-5.0*x)-1.0)
        if loss_upper_1< loss_upper_last-np.linalg.norm(grad_update)*np.linalg.norm(grad_update)*0.5*alpha:
            c_array=c_array_1
        else:
            alpha=alpha*0.3
        ...
        loss_upper_last=loss_upper.detach().numpy()/18.0
...
if __name__ == "__main__":
    ...
    plt.savefig('breast_1_my_clean.pdf') 
    plt.show()
    ...
    ltext = leg.get_texts()
    #plt.setp(ltext, fontsize=18,fontweight='bold') #设置图例字体的大小和粗细
    plt.savefig('breast_2_my_clean.pdf') 
    plt.show()