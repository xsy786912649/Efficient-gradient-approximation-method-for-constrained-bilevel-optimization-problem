...
def run(seed):
    ...
    for line in a_list:
        ...
        y=float(line2[0]) 
        x= [float(line2[i].split(':')[1]) for i in (1,2)]
    ...
    data_array_4=data_array_1*data_array_1*data_array_1

    data_array_5=np.empty((data_array_1.shape[0],0))

    for i in range(data_array_1.shape[1]):
        for j in range(data_array_1.shape[1]):
            if i<j:
                data_array_i=data_array_1[:,i]*data_array_1[:,j]*data_array_1[:,j]
                data_array_i=np.reshape(data_array_i,(-1,1))
                data_array_5=np.hstack((data_array_5,data_array_i))
                data_array_i=data_array_1[:,i]*data_array_1[:,j]*data_array_1[:,i]
                data_array_i=np.reshape(data_array_i,(-1,1))
                data_array_5=np.hstack((data_array_5,data_array_i))


    data_array_y=np.reshape(np.array(data_list)[:,-1],(-1,1))

    data_array=np.hstack((data_array_0,data_array_1,data_array_2,data_array_3,data_array_4,data_array_5,data_array_y))
    print(data_array.shape)

    np.random.seed(seed)
    np.random.shuffle(data_array)

    z_train=data_array[:600, :-1]
    y_train=data_array[:600, -1]
    z_val=data_array[600:750, :-1]
    y_val=data_array[600:750, -1]
    z_test=data_array[750:, :-1]
    y_test=data_array[750:, -1]

    c_array= torch.Tensor(600).uniform_(-7.0,-6.0)
    #c_array= torch.Tensor(600).uniform_(1.9,2.0)
    ...
    for j in range(70):
        ...
        relu = nn.LeakyReLU()
        loss_upper= torch.sum(relu(2*torch.sigmoid(-1.0*x)-1.0))
        ...
        test_loss_upper= torch.sum(relu(2*torch.sigmoid(-1.0*x1)-1.0)) 
        ...
        print("test_loss_upper: ", test_loss_upper.detach().numpy()/10.5)
        loss_upper.backward()
        ...
        test_loss_list.append(test_loss_upper.detach().numpy()/10.5)
